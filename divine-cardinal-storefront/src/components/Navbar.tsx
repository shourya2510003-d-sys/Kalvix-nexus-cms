      ...
      <header className={`sticky top-0 w-full z-40 bg-luxury-cream dark:bg-[#0c0c0c] transition-all duration-500 ${isScrolled ? 'shadow-md border-b border-luxury-gold/15' : 'shadow-none border-b border-luxury-gold/5'}`}>
        ...
        <div className={`max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between transition-all duration-500 ${isScrolled ? 'h-14 sm:h-16' : 'h-16 sm:h-20'}`}>
          ...
          <div className="text-center flex flex-col items-center justify-center flex-shrink-0 relative">
            <a href="/" className="font-serif text-base sm:text-lg md:text-xl lg:text-2xl tracking-[0.1em] text-luxury-charcoal hover:opacity-90 transition-opacity whitespace-nowrap leading-none mb-1">
              {storeName}
            </a>
            ...
            {user ? (
              <div className="relative group hidden sm:block">
                ...
                <div className="absolute top-full right-0 mt-2 w-48 bg-white border border-luxury-gold/10 shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-50">
                  <a href="/dashboard" className="block px-4 py-3 hover:bg-luxury-cream text-luxury-charcoal hover:text-luxury-gold transition-colors">
                    My Account
                  </a>
              ...
              <a href="/auth/login" className="hidden sm:flex hover:text-luxury-gold transition-colors items-center space-x-1">
                <UserIcon className="h-4 w-4" />
                <span>Account</span>
              </a>
            )}

            <a href="/blogs" prefetch={true} className="hidden xl:inline text-luxury-gold hover:text-luxury-goldDark transition-colors">
              Journal & Blogs
            </a>
        ...
        <div className={`hidden md:block overflow-x-auto scrollbar-none transition-all duration-500 ${isScrolled ? 'h-0 opacity-0 border-transparent overflow-hidden' : 'h-10 sm:h-12 border-t border-luxury-gold/10 opacity-100'}`}>
          <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-full flex items-center justify-start md:justify-center space-x-6 text-[9px] sm:text-[10px] font-serif font-normal uppercase tracking-[0.15em] text-luxury-charcoal whitespace-nowrap">
            {Object.keys(activeMenuData).map((key) => (
              ...
              >
                <a href={activeMenuData[key].link || `/shop?category=${key}`} className="hover:text-luxury-gold transition-colors py-3">
                    ...
                    <div className={`${template === 'advanced' ? 'col-span-6' : 'col-span-2'} space-y-4`}>
                      ...
                      <ul className="space-y-3 text-xs font-sans text-luxury-charcoal/80">
                        {(activeMenuData[activeMegaMenu].categories || []).map((cat: string) => (
                          <li key={cat}><a href={`/shop?category=${activeMegaMenu}`} prefetch={true} className="hover:text-luxury-gold transition-colors">{cat}</a></li>
                        ))}
                      </ul>
                    </div>
                    ...
                    <div className={`${template === 'advanced' ? 'col-span-6' : 'col-span-2'} space-y-4`}>
                      ...
                      <ul className="space-y-3 text-xs font-sans text-luxury-charcoal/80">
                        {(activeMenuData[activeMegaMenu].concerns || []).map((concern: string) => (
                          <li key={concern}><a href={`/shop?category=${activeMegaMenu}`} className="hover:text-luxury-gold transition-colors">{concern}</a></li>
                        ))}
                      </ul>
                    </div>
                    ...
                    {template === 'premium' && (
                      <>
                        ...
                        <div className="col-span-4 relative h-64 bg-luxury-cream overflow-hidden cursor-pointer group">
                          <a href={activeMenuData[activeMegaMenu].link || `/shop?category=${activeMegaMenu}`}>
                            {(() => {
                ...
                {activeMegaMenu && activeMenuData[activeMegaMenu] && template === 'simple' && (
                  <div className="col-span-12 flex space-x-8">
                     <ul className="space-y-3 text-sm font-sans text-luxury-charcoal/80">
                        {(activeMenuData[activeMegaMenu].categories || []).map((cat: string) => (
                          <li key={cat}><a href={`/shop?category=${activeMegaMenu}`} className="hover:text-luxury-gold transition-colors block py-1">{cat}</a></li>
                        ))}
                     </ul>
                  </div>
                )}
                  ...
                  <div className="flex-1 overflow-y-auto divide-y divide-luxury-gold/10 no-scrollbar pr-1">
                    {wishlist.map((item) => (
                      <div key={item.id} className="py-6 flex space-x-4">
                        <a href={`/products/${item.slug}`} onClick={() => setIsWishlistOpen(false)} className="w-20 h-24 bg-white border border-luxury-gold/10 flex-shrink-0 flex items-center justify-center p-2">
                           <img
                            src={optimizeCloudinaryUrl(item.image, 150) || 'https://images.cloudinary.com/placeholder-bottle.jpg'}
                            alt={item.name}
                            className="max-h-full max-w-full object-contain"
                        ...
                        <div className="flex-1 flex flex-col justify-between">
                          <div>
                            <a href={`/products/${item.slug}`} onClick={() => setIsWishlistOpen(false)}>
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto no-scrollbar py-4">
                ...
                <div className="px-5 mt-6 space-y-1">
                  ...
                  <a href="/shop" prefetch={true} onClick={() => setIsMobileMenuOpen(false)} className="block py-2.5 text-sm font-sans text-luxury-charcoal dark:text-luxury-cream hover:text-luxury-gold transition-colors">All Products</a>
                  <a href="/blogs" prefetch={true} onClick={() => setIsMobileMenuOpen(false)} className="block py-2.5 text-sm font-sans text-luxury-charcoal dark:text-luxury-cream hover:text-luxury-gold transition-colors">Journal & Blogs</a>
                  <a href="/pages/about" prefetch={true} onClick={() => setIsMobileMenuOpen(false)} className="block py-2.5 text-sm font-sans text-luxury-charcoal dark:text-luxury-cream hover:text-luxury-gold transition-colors">About Us</a>
                  <a href="/pages/contact" prefetch={true} onClick={() => setIsMobileMenuOpen(false)} className="block py-2.5 text-sm font-sans text-luxury-charcoal dark:text-luxury-cream hover:text-luxury-gold transition-colors">Contact</a>
                </div>
              </div>
              ...
              <div className="p-5 border-t border-luxury-gold/10 space-y-3">
                {user ? (
                  <>
                    <a href="/dashboard" onClick={() => setIsMobileMenuOpen(false)} className="flex items-center space-x-2 text-sm font-sans text-luxury-charcoal dark:text-luxury-cream">
                      <UserIcon className="h-4 w-4" />
                  ...
                  <a href="/auth/login" onClick={() => setIsMobileMenuOpen(false)} className="flex items-center space-x-2 text-sm font-sans text-luxury-charcoal dark:text-luxury-cream">
                    <UserIcon className="h-4 w-4" />
