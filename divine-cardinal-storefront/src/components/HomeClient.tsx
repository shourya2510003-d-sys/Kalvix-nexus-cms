...
import { ArrowRight, Star, Heart, ArrowLeftRight, Play, X, ChevronRight, ChevronLeft, Award, Volume2, VolumeX } from 'lucide-react';
        ...
        >
          {currentSlide.mediaType === 'video' ? (
            ...
            <>
              {/* Desktop Image */}
              <img loading="lazy"
                src={optimizeCloudinaryUrl(currentSlide.mediaUrl, 3840, true)}
                alt="Banner"
              ...
              {/* Mobile Image */}
              <img loading="lazy"
                src={optimizeCloudinaryUrl(currentSlide.mobileMediaUrl || currentSlide.mediaUrl, 1200, true)}
                alt="Banner Mobile"
...
export default function HomeClient({ banners, bestSellers, pageId = 'home', previewLayout, initialLayout }: HomeClientProps) {
  ...
  const handleReviewSubmit = async (e: React.FormEvent) => {
    ...
    if (!newReview.name.trim() || !newReview.comment.trim()) {
      alert("Please out both your name and review feedback.");
      return;
    }
                      ...
                      {bestSellers.map((product, idx) => (
                        <div key={product.id} className="snap-start flex-none w-[280px] sm:w-[320px] group flex flex-col bg-white border border-luxury-gold/10 p-4 shadow-sm hover:shadow-md transition-shadow">
                          <div className="relative h-64 bg-luxury-cream overflow-hidden flex items-center justify-center p-4 border border-luxury-gold/5">
                            ...
                            <a href={`/products/${product.slug}`} className="w-full h-full flex items-center justify-center">
                              <img loading="lazy"
                            ...
                            <button 
                              onClick={(e) => {
                                ...
                                toggleWishlist({
                                  ...
                                  price: typeof product.basePrice === 'string' ? parseFloat(product.basePrice.replace(/[^\d.]/g, '')) : product.basePrice,
                                  category: product.category || 'Product'
                                });
                              }}
                              className="absolute bottom-3 right-3 bg-white/90 p-2 rounded-full border border-luxury-gold/10 hover:bg-white transition-colors cursor-pointer z-10"
                            >
                              <Heart className={`h-3.5 w-3.5 transition-colors ${isInWishlist(product.id) ? 'fill-luxury-gold text-luxury-gold' : 'text-luxury-charcoal hover:fill-red-600 hover:text-red-600'}`} />
                            </button>
                          </div>
                ...
                <section key={section.id} className="max-w-7xl mx-auto px-6 sm:px-8 space-y-12">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-6">
                    {section.data.items.map((con: any) => (
                      <a href={con.link || '/shop'} key={con.name} className="group relative flex flex-col items-center cursor-pointer">
                        ...
                        <div key={idx} className="flex-shrink-0 w-64 bg-white border border-luxury-gold/15 rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow group">
                          ...
                          <div className="p-4 space-y-1.5 text-left bg-[#FAF9F6]/30">
                            ...
                            <div className="flex items-center justify-between pt-1 border-t border-luxury-gold/5">
                              ...
                              <a href={tale.shopLink || "/shop"} className="text-[9px] uppercase font-sans tracking-widest text-[#008060] font-semibold hover:underline">
                                Shop &gt;
                              </a>
                            </div>
                          </div>
                          ...
                          <div key={product.id} className="snap-start flex-none w-[280px] sm:w-[320px] group flex flex-col bg-white border border-luxury-gold/10 p-4 shadow-sm hover:shadow-md transition-shadow">
                            <div className="relative h-64 bg-luxury-cream overflow-hidden flex items-center justify-center p-4 border border-luxury-gold/5">
                              ...
                              <button 
                                ...
                                className="absolute bottom-3 right-3 bg-white/90 p-2 rounded-full border border-luxury-gold/10 hover:bg-white transition-colors cursor-pointer z-20"
                              >
                                <Heart className={`h-3.5 w-3.5 transition-colors ${isInWishlist(product.id) ? 'fill-luxury-gold text-luxury-gold' : 'text-luxury-charcoal hover:fill-red-600 hover:text-red-600'}`} />
                              </button>
                            </div>
                ...
                <section key={section.id} className="max-w-7xl mx-auto px-6 sm:px-8 space-y-24">
                  <div className="space-y-8">
                    <div className="flex justify-between items-end border-b border-luxury-gold/15 pb-4">
                      ...
                      <a href={section.data.buttonLink} className="text-[10px] uppercase tracking-[0.25em] text-luxury-gold hover:text-luxury-goldDark font-sans font-medium transition-colors">
                        {section.data.buttonText}
                      </a>
                    </div>
                ...
                <section key={section.id} className="bg-[#FAF9F6] border-y border-luxury-gold/15">
                  <div className="max-w-7xl mx-auto px-6 sm:px-8 py-20">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
                      <div className="relative aspect-[4/5] sm:h-[400px] lg:h-[500px] sm:aspect-auto rounded-t-full overflow-hidden border-8 border-white shadow-xl max-w-[400px] mx-auto lg:max-w-none">
