...
import { Save, Plus, Trash2, Upload, GripVertical, ArrowUp, ArrowDown } from 'lucide-react';
...
export default function GlobalUIBuilder() {
  ...
  const moveMenuItem = (idx: number, direction: 'up' | 'down') => {
    if (!headerConfig.menuItems) return;
    if (direction === 'up' && idx === 0) return;
    if (direction === 'down' && idx === headerConfig.menuItems.length - 1) return;
  ...
  const moveFooterColumn = (idx: number, direction: 'up' | 'down') => {
    if (!footerConfig.columns) return;
    if (direction === 'up' && idx === 0) return;
    if (direction === 'down' && idx === footerConfig.columns.length - 1) return;
          ...
          <div>
            ...
            <div className="space-y-4">
              {(headerConfig.menuItems || []).map((item: any, idx: number) => (
                <div key={item.id} className="border border-gray-200 p-4 rounded bg-gray-50 relative group">
                  ...
                  <div className="absolute -top-3 -right-3 flex items-center space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button 
                      ...
                      disabled={idx === 0}
                      className="bg-white text-gray-600 p-1.5 rounded-full hover:bg-gray-100 shadow border border-gray-200 disabled:opacity-50"
                    >
                      <ArrowUp className="w-3.5 h-3.5" />
                    </button>
                    <button 
                      onClick={() => moveMenuItem(idx, 'down')}
                      disabled={idx === (headerConfig.menuItems?.length || 0) - 1}
                      className="bg-white text-gray-600 p-1.5 rounded-full hover:bg-gray-100 shadow border border-gray-200 disabled:opacity-50"
                    >
                    ...
                    <button 
                      ...
                      className="bg-red-100 text-red-600 p-1.5 rounded-full hover:bg-red-200 shadow ml-2"
                    >
        ...
        <div className="space-y-6 bg-white p-6 rounded border border-gray-200 shadow-sm">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            {(footerConfig.columns || []).map((col: any, idx: number) => (
              <div key={col.id} className="border border-gray-200 p-4 rounded bg-gray-50 relative group">
                <div className="absolute -top-3 -right-3 flex items-center space-x-1 opacity-0 group-hover:opacity-100 transition-opacity z-10">
                  <button 
                    onClick={() => moveFooterColumn(idx, 'up')}
                    disabled={idx === 0}
                    className="bg-white text-gray-600 p-1.5 rounded-full hover:bg-gray-100 shadow border border-gray-200 disabled:opacity-50"
                  >
                    <ArrowUp className="w-3.5 h-3.5" />
                  ...
                  <button 
                    onClick={() => moveFooterColumn(idx, 'down')}
                    disabled={idx === (footerConfig.columns?.length || 0) - 1}
                    className="bg-white text-gray-600 p-1.5 rounded-full hover:bg-gray-100 shadow border border-gray-200 disabled:opacity-50"
                  >
                    <ArrowDown className="w-3.5 h-3.5" />
                  </button>
                  <button 
                    onClick={() => {
                      const newCols = footerConfig.columns.filter((_: any, i: number) => i !== idx);
                      setFooterConfig({ ...footerConfig, columns: newCols });
                    }}
                    className="bg-red-100 text-red-600 p-1.5 rounded-full hover:bg-red-200 shadow ml-2"
                  >
