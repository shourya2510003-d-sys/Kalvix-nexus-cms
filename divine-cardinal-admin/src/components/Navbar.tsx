...
import { ShoppingBag, Search, User as UserIcon, X, Plus, Minus, Trash2, MapPin, ChevronDown, Sun, Moon, ArrowLeft, Menu, Heart } from 'lucide-react';
...
export default function Navbar() {
  ...
  const [storeName, setStoreName] = useState(process.env.NEXT_PUBLIC_STORE_NAME || 'DIVINE CARDINAL');

  useEffect(() => {
    if (typeof window !== 'undefined') {
      const host = window.location.hostname;
      const subdomain = host.split('.')[0];
      if (subdomain && subdomain !== 'www' && subdomain !== 'divinecardinal' && subdomain !== 'kalvixnexus') {
        setStoreName(subdomain.toUpperCase());
      }
    }
  }, []);
  ...
  const { wishlistCount, wishlist, isWishlistOpen, setIsWishlistOpen, removeItem: removeWishlistItem } = useWishlist();
      ...
      <header className={`sticky top-0 w-full z-40 bg-luxury-cream dark:bg-[#0c0c0c] transition-all duration-500 ${isScrolled ? 'shadow-md border-b border-luxury-gold/15' : 'shadow-none border-b border-luxury-gold/5'}`}>
        ...
        <div className={`max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between transition-all duration-500 ${isScrolled ? 'h-14 sm:h-16' : 'h-16 sm:h-20'}`}>
          ...
          <div className="text-center flex flex-col items-center justify-center flex-shrink-0 relative">
            <Link to="/" className="font-serif text-base sm:text-lg md:text-xl lg:text-2xl tracking-[0.1em] text-luxury-charcoal hover:opacity-90 transition-opacity whitespace-nowrap leading-none mb-1">
              {storeName}
            </Link>
            <span className="hidden lg:block text-[8px] md:text-[9px] uppercase tracking-[0.2em] font-sans text-luxury-gold">A path of natural well being</span>
          </div>
          ...
          <div className="flex-1 flex items-center justify-end space-x-4 sm:space-x-6 text-[10px] sm:text-[11px] uppercase tracking-widest font-serif font-normal text-luxury-charcoal dark:text-luxury-cream">
            ...
            {user ? (
              <div className="relative group hidden sm:block">
                <button className="flex hover:text-luxury-gold transition-colors items-center space-x-1">
                  <UserIcon className="h-4 w-4" />
                  <span className="max-w-[80px] truncate">Hi, {user.firstName || 'User'}</span>
                </button>
                <div className="absolute top-full right-0 mt-2 w-48 bg-white border border-luxury-gold/10 shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-50">
                  <Link to="/dashboard" className="block px-4 py-3 hover:bg-luxury-cream text-luxury-charcoal hover:text-luxury-gold transition-colors">
                    My Account
                  </Link>
                  <button onClick={logout} className="block w-full text-left px-4 py-3 hover:bg-luxury-cream text-luxury-charcoal hover:text-luxury-gold transition-colors border-t border-luxury-gold/10">
                    Logout
                  </button>
                </div>
              </div>
            ) : (
              <Link to="/auth/login" className="hidden sm:flex hover:text-luxury-gold transition-colors items-center space-x-1">
                <UserIcon className="h-4 w-4" />
                <span>Account</span>
              </Link>
            )}

            <Link to="/blogs" prefetch={true} className="hidden xl:inline text-luxury-gold hover:text-luxury-goldDark transition-colors">
              Journal & Blogs
            </Link>

            <button onClick={() => setIsWishlistOpen(true)} className="relative flex items-center space-x-1.5 hover:text-luxury-gold transition-colors">
              <div className="relative">
                <Heart className="h-5 w-5" />
                {wishlistCount > 0 && (
                  <span className="absolute -top-1.5 -right-1.5 bg-luxury-gold text-white text-[8px] w-4.5 h-4.5 rounded-full flex items-center justify-center font-serif font-normal">
                    {wishlistCount}
                  </span>
                )}
              </div>
        ...
        <div className={`hidden md:block overflow-x-auto scrollbar-none transition-all duration-500 ${isScrolled ? 'h-0 opacity-0 border-transparent overflow-hidden' : 'h-10 sm:h-12 border-t border-luxury-gold/10 opacity-100'}`}>
          <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-full flex items-center justify-start md:justify-center space-x-6 text-[9px] sm:text-[10px] font-serif font-normal uppercase tracking-[0.15em] text-luxury-charcoal whitespace-nowrap">
            {Object.keys(activeMenuData).map((key) => (
              ...
              >
                <Link to={activeMenuData[key].link || `/shop?category=${key}`} className="hover:text-luxury-gold transition-colors py-3">
                    ...
                    <div className={`${template === 'advanced' ? 'col-span-6' : 'col-span-2'} space-y-4`}>
                      ...
                      <ul className="space-y-3 text-xs font-sans text-luxury-charcoal/80">
                        {(activeMenuData[activeMegaMenu].categories || []).map((cat: string) => (
                          <li key={cat}><Link to={`/shop?category=${activeMegaMenu}`} prefetch={true} className="hover:text-luxury-gold transition-colors">{cat}</Link></li>
                        ))}
                      </ul>
                    </div>
                    ...
                    <div className={`${template === 'advanced' ? 'col-span-6' : 'col-span-2'} space-y-4`}>
                      ...
                      <ul className="space-y-3 text-xs font-sans text-luxury-charcoal/80">
                        {(activeMenuData[activeMegaMenu].concerns || []).map((concern: string) => (
                          <li key={concern}><Link to={`/shop?category=${activeMegaMenu}`} className="hover:text-luxury-gold transition-colors">{concern}</Link></li>
                        ))}
                      </ul>
                    </div>
                    ...
                    {template === 'premium' && (
                      <>
                        ...
                        <div className="col-span-4 relative h-64 bg-luxury-cream overflow-hidden cursor-pointer group">
                          <Link to={activeMenuData[activeMegaMenu].link || `/shop?category=${activeMegaMenu}`}>
                            {(() => {
                ...
                {activeMegaMenu && activeMenuData[activeMegaMenu] && template === 'simple' && (
                  <div className="col-span-12 flex space-x-8">
                     <ul className="space-y-3 text-sm font-sans text-luxury-charcoal/80">
                        {(activeMenuData[activeMegaMenu].categories || []).map((cat: string) => (
                          <li key={cat}><Link to={`/shop?category=${activeMegaMenu}`} className="hover:text-luxury-gold transition-colors block py-1">{cat}</Link></li>
                        ))}
                     </ul>
                  </div>
                )}
      ...
      <AnimatePresence>
        {isWishlistOpen && (
          <>
            ...
            >
              <div className="flex items-center justify-between pb-6 border-b border-luxury-gold/15">
                <h3 className="font-serif text-xl tracking-wide">Wishlist ({wishlistCount})</h3>
                <button onClick={() => setIsWishlistOpen(false)} className="hover:text-luxury-gold transition-colors">
                  <X className="h-6 w-6" />
                </button>
              </div>
              ...
              {wishlist.length === 0 ? (
                <div className="flex-1 flex flex-col items-center justify-center text-center space-y-4">
                  <Heart className="h-12 w-12 text-luxury-gold/50" />
                  ...
                  <div className="flex-1 overflow-y-auto divide-y divide-luxury-gold/10 no-scrollbar pr-1">
                    {wishlist.map((item) => (
                      <div key={item.id} className="py-6 flex space-x-4">
                        <Link to={`/products/${item.slug}`} onClick={() => setIsWishlistOpen(false)} className="w-20 h-24 bg-white border border-luxury-gold/10 flex-shrink-0 flex items-center justify-center p-2">
                           <img
                            src={optimizeCloudinaryUrl(item.image, 150) || 'https://images.cloudinary.com/placeholder-bottle.jpg'}
                            alt={item.name}
                            className="max-h-full max-w-full object-contain"
                        ...
                        <div className="flex-1 flex flex-col justify-between">
                          <div>
                            <Link to={`/products/${item.slug}`} onClick={() => setIsWishlistOpen(false)}>
                              <h4 className="font-serif text-sm tracking-wide hover:text-luxury-gold transition-colors">{item.name}</h4>
                            </Link>
                            <p className="text-xs text-luxury-gold uppercase mt-1">{item.category}</p>
                          </div>
                          <div className="flex items-center justify-between mt-4">
                            <span className="text-sm font-serif">{formatPrice(item.price)}</span>
                            <button
            ...
            >
              ...
              <div className="flex items-center justify-between p-5 border-b border-luxury-gold/10">
                <div className="flex flex-col">
                  <span className="font-serif text-lg tracking-[0.1em] text-luxury-charcoal dark:text-luxury-cream">
                    {storeName}
                  </span>
                </div>
              ...
              <div className="flex-1 overflow-y-auto no-scrollbar py-4">
                ...
                <div className="px-5 space-y-1">
                  {Object.keys(activeMenuData).map((key) => (
                    <Link
                      key={key}
                      to={activeMenuData[key].link || `/shop?category=${key}`}
                      onClick={() => setIsMobileMenuOpen(false)}
                ...
                <div className="px-5 mt-6 space-y-1">
                  ...
                  <Link to="/shop" prefetch={true} onClick={() => setIsMobileMenuOpen(false)} className="block py-2.5 text-sm font-sans text-luxury-charcoal dark:text-luxury-cream hover:text-luxury-gold transition-colors">All Products</Link>
                  <Link to="/blogs" prefetch={true} onClick={() => setIsMobileMenuOpen(false)} className="block py-2.5 text-sm font-sans text-luxury-charcoal dark:text-luxury-cream hover:text-luxury-gold transition-colors">Journal & Blogs</Link>
                  <Link to="/pages/about" prefetch={true} onClick={() => setIsMobileMenuOpen(false)} className="block py-2.5 text-sm font-sans text-luxury-charcoal dark:text-luxury-cream hover:text-luxury-gold transition-colors">About Us</Link>
                  <Link to="/pages/contact" prefetch={true} onClick={() => setIsMobileMenuOpen(false)} className="block py-2.5 text-sm font-sans text-luxury-charcoal dark:text-luxury-cream hover:text-luxury-gold transition-colors">Contact</Link>
                </div>
              </div>
              ...
              <div className="p-5 border-t border-luxury-gold/10 space-y-3">
                {user ? (
                  <>
                    <Link to="/dashboard" onClick={() => setIsMobileMenuOpen(false)} className="flex items-center space-x-2 text-sm font-sans text-luxury-charcoal dark:text-luxury-cream">
                      <UserIcon className="h-4 w-4" />
                  ...
                  <Link to="/auth/login" onClick={() => setIsMobileMenuOpen(false)} className="flex items-center space-x-2 text-sm font-sans text-luxury-charcoal dark:text-luxury-cream">
                    <UserIcon className="h-4 w-4" />
